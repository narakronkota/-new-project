...
const Allrooms = () => {
  ...
  return (
    <div className='  flex flex-col-reverse lg:flex-row items-start justifly-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
        <div>
            <div>
        ...
        {/******df */}
        <div className='  bg-white w-80  border border-gray-300 text-gray-600 max-lg:mb-8 min-lg: mt-25 ms-8 '>
            <div className={`flex items-center justify-between px-5 py-2.5 min-lg:border-b border-gray-300 ${openFilters && "border-b"}`}>
                   <p className='text-base font-medium text-gray-800'>FIlTERS</p>
        <div className='text-xs cursor-pointer'>
            <span onClick={()=> setOpenFilters(!openFilters)} className='lg:hidden'>
                {openFilters ? 'HIDE' : 'SHOW'}</span>
            <span className='hidden   text-sm  ml-40 lg:block '>CLEAR</span>
         </div>
     
        </div>
        <div className={`${openFilters ? 'h-auto' : "h-0 lg:h-auto overflow-hidden transition-all duration-700"}`}>
            <div className='px-5 pt-5'>
                <p className='font-medium text-gray-800 pb-2'>Poppular filters</p>
